...
if (isset($_SESSION['wiki_id'])) {
    ...
    if (!isset($_SESSION['email']) || is_null($_SESSION['email']) || !is_string($_SESSION['email']) || strlen($_SESSION['email']) < 10) {
        $user_email = $_SESSION['net_id'] + "@dartmouth.edu";
    } else {
    ...
    $query = "SELECT wiki_name, description, root_folder, app_folder, logo_link,
                ...
                IFNULL((SELECT SUM(wc) FROM comments where wiki_id='$wiki_id'),0) AS comment_words,
                IFNULL((SELECT SUM(wc) FROM posts where wiki_id='$wiki_id'),0) AS post_words,
                IFNULL((SELECT SUM(wc) FROM micro_posts where wiki_id='$wiki_id'),0) AS micro_words
			FROM wikis w
        	LEFT JOIN users u
              ON u.net_id='$net_id'
            LEFT JOIN logins l
    ...
    $login_result->canvas_course_id = 0;
